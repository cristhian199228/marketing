  ...
  <div>
    ...
    >
      ...
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" @click="$store.commit('solicitud/LIMPIAR_FILTROS')" text>LIMPIAR</v-btn>
...
export default {
  name: "FiltersUsuario",
  components: {Estado, Formato, Proyecto, Codigo, FechaFin, FechaInicio, Prioridad},
  data() {
    return {
      menu: false,
      module: 'solicitud'
  ...
  methods: {
    aplicarFiltros() {
      this.$store.commit('solicitud/SET_FIRST_PAGE')
      this.menu = false
